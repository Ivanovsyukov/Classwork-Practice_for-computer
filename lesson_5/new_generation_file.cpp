...
int main(int argc, char* argv[]){
    ...
    std::fstream fs;
    fs.open("D:\\program\\Git\\Classwork-Practice_for-computer\\lesson_5\\file_generate_new.txt", std::ios_base::out | std::ios_base::binary);
    if (!fs.is_open()){
    ...
    for (size_t k=0; k<filesize/sizeof(buffer); ++k){
        for (size_t j=0; j<sizeof(buffer); ++j){
            buffer[j]=static_cast<char>(rand());
        }
